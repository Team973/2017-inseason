...
public:
     ...
     *
     * @param target represents the target the pid controler will
     * try to achieve.
     */
     ...
     *
     * @return returns the current target that the pid controller is trying
     * to achieve.
     */
    double GetTarget();
     ...
     * of error*sec, whatever that means to you.  This cap works in absolute
     * terms; so the integral of error will be bound between positive |icap|
     * and negative |icap|
     *
     * @param icap maximum integral of error that may occur in the units of
     * error*sec
     */
    ...
    /**
     * Calculate the output of the PID controller.  This form uses the FPGA
     * to determine current time and compares it to the time the controller
     * was last called at.
     *
     * @param actual current position/speed/state of the system being
     * controlled
     *
     * @return Calculated ideal value to send to output device to achieve
     * the desired target.
     */
     ...
     * parameter (unit in milliseconds) to determine time and compares it to
     * the time the controller was last called with.
     *
     * @param actual current position/speed/state of the system being
     * controlled.
    ...
    /**
     * Set the previous output of the PID controller.  This can be useful
     * for setting the start speed for speed control.
     *
     * @param previous output (feed forward) value.
     */
